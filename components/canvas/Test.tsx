...
import { NODE_COLORS, WIRE_COLOR_ACTIVE, WIRE_COLOR_INACTIVE } from '../../constants';
...
const CircuitNodeDisplay: React.FC<CircuitNodeDisplayProps> = ({
  ...
  if (node.type === NodeType.Output) {
    ...
    mainBgColor = NODE_COLORS[node.type] || colorMap.slate600;
  }
  const borderColor = node.value ? WIRE_COLOR_ACTIVE : WIRE_COLOR_INACTIVE;
  const labelTextColor = (mainBgColor === colorMap.emerald400 || mainBgColor === NODE_COLORS[NodeType.Input]) ? colorMap.slate900 : colorMap.slate100;
  
  const showGateIcon = !pathGeneratorFn;
  const showsInverterBubbleComponent = [NodeType.NAND, NodeType.NOR, NodeType.XNOR, NodeType.NOT].includes(node.type);
